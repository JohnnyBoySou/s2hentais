...
export default function Match({ navigation, route, ...props }) {
  ...
  async function getDados(valor){
    ...
    setLoading(true)
    if(valor.alugar){
      await Axios.get(`https://maisconectados.com/wp-json/feed/alugar?${valor.item1}${max}`, {
        headers: headers
  ...
  const renderItem = useCallback(({ item, index }) => (
    ...
    <TouchableRipple onPress={() => pegarPost(item)} style={{flex: 1,borderRadius: 12, }} rippleColor="#FFF" borderless={true}>
      ...
      <Like onPress={() => pegarPost(item)}>
        <AntDesign name="heart" size={42} color="#fff" />
      </Like>
      ...
      <Description>
        ...
        <View style={{flexDirection: 'row', flexWrap: 'wrap', marginTop: 10, }}>
          <Tag>Próximo a Escola</Tag>
          <Tag>Posto de Saúde</Tag>
          <Tag>Posto de Gasolina</Tag>
        </View>
      </Description>      
...
return (
  ...
  <View style={{flexDirection: 'row', justifyContent: 'center', alignSelf: 'center', marginTop: 20,}}>
    ...
    { loading == false && <>
    {ofertas > 0 && <><Ofertas>Encontre seu Match</Ofertas>
      <AnimatePresence>
      <MotiView
